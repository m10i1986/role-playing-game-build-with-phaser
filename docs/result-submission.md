...
| 方式 | イベント | 送信内容 | 主な宛先 |
| --- | --- | --- | --- |
| [PhaserWorks向け送信](#phaserworks向け送信) | `EventTypeEnum.SendGameResultWithPhaserWorks` | 回答履歴・プレイ時間・`result`(成否/スコア)の全データ | 自社サーバー(PhaserWorks) |
| [Power Automate向け送信](#power-automate向け送信) | `EventTypeEnum.SendGameResultWithPowerAutomate` | `preferredUsername`・成否・スコア・プレイ時間・追加変数 | Power AutomateのHTTP Webhookトリガー |
...
シナリオ（Timeline DSL）内で `SendGameResultWithPhaserWorks` イベントを配置したタイミングで送信します。
...
| フィールド | 型 | 説明 |
| --- | --- | --- |
| `type` | `"choice" \| "multi_choice" \| "sort_order" \| "input_number"` | 回答の種類。 |
| `key` | `string` | 回答対象を識別するキー。`choice`/`multi_choice`/`sort_order` は遷移先のタイムラインID、`input_number` は入力値を保存する変数名。 |
| `value` | `string \| number \| boolean \| (string \| number \| boolean)[]` | 回答内容。`choice` は選択したテキスト、`multi_choice` は選択した全テキストの配列、`sort_order` は判定時点の並び順（表示テキストの配列）、`input_number` は入力された数値。 |
...
```
https://example.com/game/?preferredUsername=<プレイヤーを識別するメールアドレス(URLエンコード)>
```

| パラメータ | 必須 | 説明 |
| --- | --- | --- |
| `preferredUsername` | ○ | プレイヤーを識別するメールアドレス(preferred_username)。未指定の場合、送信は中断されコンソールにエラーが出力される。 |
...
`variables` に変数ストア上のkeyを指定すると、`result_success`/`result_score` 以外の任意のnumber変数・list変数を追加で送信できます（[`variables`（追加の変数データ）](#variables追加の変数データ)を参照）。

```ts
{
  event: EventTypeEnum.SendGameResultWithPowerAutomate,
  url: "https://prod-00.japaneast.logic.azure.com/...",
  variables: ["combo_count", "correct_answers"], // combo_countはnumber変数、correct_answersはlist変数の例
},
```
...
| フィールド | 型 | 説明 |
| --- | --- | --- |
| `preferredUsername` | `string` | 起動URLで指定した `preferredUsername`。 |
...
| `variables` | `Record<string, number \| VariableValue[] \| null> \| undefined` | イベントの `variables` で指定したkeyに対応する追加データ。イベント側で `variables` を指定しない場合、このフィールド自体が省略されます。[`variables`（追加の変数データ）](#variables追加の変数データ)を参照。 |

### `variables`（追加の変数データ）

イベントの `variables: string[]` に指定した各keyについて、以下の優先順位で値を解決します。

1. `SetVariable`/`IncrementVariable` 等で設定したnumber変数として存在する場合は、その数値をそのまま使用する。
2. number変数として存在しない場合、`SetVariableList`/`PushVariableList` 等で設定したlist変数として存在すれば、その配列を使用する。
3. どちらにも該当しない場合（未設定、またはstring/boolean変数を指定した場合）は `null` を格納する。

```ts
// number変数とlist変数を用意しておく
{ event: EventTypeEnum.SetVariable, key: "combo_count", value: 5 },
{ event: EventTypeEnum.SetVariableList, key: "correct_answers", values: ["バナナ", "リンゴ"] },

// エンディング直前などでPower Automateへ送信する
{
  event: EventTypeEnum.SendGameResultWithPowerAutomate,
  url: "https://prod-00.japaneast.logic.azure.com/...",
  variables: ["combo_count", "correct_answers"],
},
```
...
- [src/classes/variable_store.ts](../src/classes/variable_store.ts) — シナリオの `SetVariable`/`SetVariableList` イベントで設定した変数・リスト変数を保持する変数ストア。`sendGameResultWithPowerAutomate` の `variables` はここから値を読み取る。
...
- [src/classes/timeline_player.ts](../src/classes/timeline_player.ts) — 選択肢・並び替え・数値入力のたびに `recordAnswer()` を呼び出して回答を記録し、`SendGameResultWithPhaserWorks`/`SendGameResultWithPowerAutomate` イベント実行時にそれぞれの送信関数を呼び出す。
...
- Power Automate向け送信は暗号化を行わないため、機密性の高い情報を含めないでください。また `preferredUsername` はURLクエリパラメータとして平文で渡されるため、PhaserWorks向け送信と同様にURL自体の漏洩に注意してください。
