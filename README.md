...
3. src/senario.ts を作成し、シナリオを記述します。
   - src/senario.ts が存在しない場合、`pnpm dev` / `pnpm build` 実行時に src/senario.sample.ts が自動的にコピーされ、サンプルシナリオで動作確認できます。
...
5. ブラウザで `http://localhost:5173/` にアクセスします。(テスト環境)
